...
<a class="dropdown-item" href="/cultural/logout.php">Logout</a>
</div>
</div>
</div>
</div>
...
<?php
$connect5 = mysqli_connect("localhost", "root", "", "cultural");
$sql=mysqli_query($connect5,"select * from user where role='1'");
...
<?php
$connect5 = mysqli_connect("localhost", "root", "", "cultural");
$sql=mysqli_query($connect5,"select * from user where role='2'");
if(mysqli_num_rows($sql)>0){
while($app=mysqli_fetch_assoc($sql)){
$id=$app['id'];
$name=$app['name'];
...
<td><?php echo $name ?></td>
<form method="POST" action="/cultural/api/vote.php">
