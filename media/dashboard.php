...
<a class="dropdown-item" href="/media/logout.php">Logout</a>
</div>
</div>
</div>
</div>
...
$sql=mysqli_query($connect2,"select * from user where role='1'");
...
<?php
$connect2 = mysqli_connect("localhost", "root", "", "media");
$sql=mysqli_query($connect2,"select * from user where role='2'");
if(mysqli_num_rows($sql)>0){
while($app=mysqli_fetch_assoc($sql)){
$id=$app['id'];
$name=$app['name'];
...
<td><?php echo $name ?></td>
<form method="POST" action="/media/api/vote.php">
