...
<a class="dropdown-item" href="/technical/logout.php">Logout</a>
</div>
</div>
</div>
</div>
...
$sql=mysqli_query($connect3,"select * from user where role='1'");
...
<?php
$connect3 = mysqli_connect("localhost", "root", "", "technical");
$sql=mysqli_query($connect3,"select * from user where role='2'");
if(mysqli_num_rows($sql)>0){
while($app=mysqli_fetch_assoc($sql)){
$id=$app['id'];
$name=$app['name'];
...
<td><?php echo $name ?></td>
<form method="POST" action="/technical/api/vote.php">
